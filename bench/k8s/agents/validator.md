...
4. **Egress is physically possible.** Compare `registry_tx_bytes_delta` against the dataset size times the number of workers (2). The dataset is 11.46 GiB; do NOT derive it from `deploy/k8s/performance/input/dataset_a.tsv`, whose size column has 9 zeroed cells and sums to only 3.68 GiB. A delta of a few hundred bytes means the probe read the wrong network interface; a delta far above ~23 GiB deserves an explanation.

...
Both are off by default, so the emitted `ctr images import` is unchanged unless
...
`--no-unpack` for intermediate prefixes was measured and **not** added: it moves the
unpack work out of the window that overlaps the download and into the final import,
growing the non-overlapped tail from 0.7 s to 11.6 s for no gain in total time.
...
- client.http.imageTimeoutMin = PT5M
- client.http.imageTimeoutMax = PT30M

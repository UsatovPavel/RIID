...
- **`riid-image-sizes.json`** — tar size bucket counts and load duration p50/p95 by bucket; **`sort_idx`** tag on RIID metrics defines Grafana bar/table order (see `ImageSizeBucket`).

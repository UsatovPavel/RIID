...
**Makefile (repo root):** `make metrics-stack-create` — tear down old stack if needed, then start VM + Grafana + vmagent; `make metrics-stack-update` — `docker restart grafana` after editing JSON under `grafana/dashboards/`; `make metrics-stack-down` — remove all stack containers.

...
class RegistryGhcrStatusTest {
    ...
    void pingReturns200() throws Exception {
        ...
        assertTrue(Set.of(200, 401).contains(status), "Expected 200 or 401 from GHCR /v2/, got " + status);
    }
        ...
        for (int i = 0; i < 5; i++) {
            try {
                HttpResponse<Void> resp = CLIENT.send(
                        HttpRequest.newBuilder(URI.create(GHCR + path)).timeout(Duration.ofSeconds(10)).GET().build(),

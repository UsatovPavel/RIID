...
#
# PodmanRuntimeAdapter itself does not use this driver: it speaks Libpod HTTP
# directly over CONTAINER_HOST. Both paths still operate on the same node store.
#
...
_podman_node_exec() {
  ...
  if [[ -z "$node_pod" ]]; then
    echo "no Running podman-node pod on node=$node for RIID pod=$riid_pod" >&2
    return 1
  fi

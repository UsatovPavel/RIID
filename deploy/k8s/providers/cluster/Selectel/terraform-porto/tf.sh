...
# Same contract as ../terraform/tf.sh, different module.
set -euo pipefail

HERE="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
MKS_DIR="$(cd "$HERE/../terraform" && pwd)"
...
# The resolver lives with the MKS module; both stands sit in the same project.
if project_id="$(ENV_FILE="$ENV_FILE" "$MKS_DIR/resolve-project-id.sh" 2>/dev/null)"; then

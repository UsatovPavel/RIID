#!/usr/bin/env bash
# На всех worker-нодах через podman-node выполняется podman login на host из
# providers/registry/client/selectel.yaml. В RIID-образе podman CLI отсутствует.
set -euo pipefail
